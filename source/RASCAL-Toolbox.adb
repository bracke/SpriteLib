...
package body RASCAL.Toolbox is
   ...
   function Create_Object (Template : in string;
                           Flags    : in Creation_Type := From_Template) return Object_ID is

      Register             : aliased Kernel.swi_regs;
      Error                : oserror_access;
      Template_0           : string             := Template & ASCII.NUL;
   begin
      Register.R(0) := Creation_Type'Pos(Flags);
      Register.R(1) := Adr_To_Int(Template_0'Address);

      Error := Kernel.swi(Toolbox_CreateObject,Register'Access,Register'Access);

      if Error /= null then
         pragma Debug(Reporter.Report("Toolbox.Create_Object: " & To_Ada(Error.ErrMess)));
